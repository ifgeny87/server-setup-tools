...
systemctl daemon-reload # для загрузки нового конфига ssh
systemctl restart ssh
if [ $? -ne 0 ]; then
    echo "Ошибка: не удалось перезапустить SSH‑сервер. Проверьте: journalctl -u sshd"
    exit 1
fi
...
Двухфакторная аутентификация: ключ + пароль
